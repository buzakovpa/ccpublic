...
namespace CC.WEB.Infrastructure.Helpers
{
    ...
    public static class HtmlHelpers
    {
        ...
        public static MvcHtmlString EnumDropDownList<TEnumType>(this HtmlHelper htmlHelper, string name, TEnumType value, object htmlAttributes)
        {
            var selectItems = GetSelectItemsForEnum(typeof(TEnumType), value);
            return htmlHelper.DropDownList(name, selectItems, htmlAttributes: htmlAttributes);
        }
    ...
    ) where TModel : class
        {
            ...
            if (includeNoneOption) {
                bool any = false;
                foreach (SelectListItem e in enumNames) {
                    if (e.Value.ToLowerInvariant() == "none") {
                        any = true;
                        break;
                    }
                }
                if (!any)
                    enumNames.Add(new SelectListItem{Text = "None"});
            }
            ...
            foreach (var enumName in enumNames) {
                ...
                if (includeNoneOption && enumName.Value == "None")
                    value = new SelectListItem {Value = string.Empty};
